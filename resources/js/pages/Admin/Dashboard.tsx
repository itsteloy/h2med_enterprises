...
import { 
    ...
    RefreshCw, Calendar, ShoppingCart, Truck, Clock, PieChart, 
    Activity, CheckCircle, XCircle
} from 'lucide-react';

// Custom Peso Icon Component
const PesoIcon = ({ className }: { className?: string }) => (
    <span className={className} style={{ fontSize: '1.5em', fontWeight: 'bold' }}>₱</span>
);
import { useState, useEffect } from 'react';
import { dashboardService, DashboardOverview, DashboardCharts } from '../../../services/dashboardServices';
import { deliveryService, Delivery } from '../../../services/deliveryServices';
...
const DeliverySummaryCard = ({ 
    ...
    return (
        <div className="bg-white rounded-2xl shadow-lg p-5 border border-gray-100 hover:shadow-xl transition-shadow">
            <div className="flex items-center justify-between">
                <div className="flex-1">
                    <p className="text-xs font-medium text-gray-500 mb-1">Delivery Total Value</p>
                    {loading ? (
                        <div className="animate-pulse">
                            <div className="h-7 bg-gray-200 rounded w-20 mb-1"></div>
                        ...
                        <>
                            ...
                            <p className="text-gray-600 text-xs">{deliveryCount} total deliveries</p>
                        </>
                    )}
                </div>
...
const RecentActivityCard = ({ 
    ...
    const getActivityDisplay = (item: any) => {
        if (type === 'movements') {
            return {
                        ...
                        return (
                            <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition-colors">
                                <div className="flex items-center">
                                    <div className="w-2 h-2 bg-[#ff1a1a] rounded-full mr-3"></div>
                                    <div>
...
const ChartCard = ({ 
    ...
    const renderChart = () => {
        ...
        if (type === 'category' || type === 'supplier') {
            const maxValue = Math.max(...data.map(d => type === 'category' ? d.total_stock : d.total_value));
            
            return (
                <div className="space-y-3">
                    {data.slice(0, 5).map((item, index) => {
                        const value = type === 'category' ? item.total_stock : item.total_value;
                        ...
                        return (
                            <div key={index} className="space-y-2">
                                ...
                                <div className="w-full bg-gray-200 rounded-full h-2">
                                    <div 
                                        className="bg-[#ff1a1a] h-2 rounded-full transition-all duration-300" 
                ...
                {data.slice(0, 7).map((item, index) => (
                    <div key={index} className="flex justify-between items-center p-2 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex space-x-4 text-sm">
                            <span className="text-green-600">In: {item.stock_in}</span>
                            <span className="text-red-600">Out: {item.stock_out}</span>
                            <span className={`font-medium ${item.net_change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
...
export default function AdminDashboard() {
    ...
    const [deliveryStats, setDeliveryStats] = useState<{ totalValue: number; totalCount: number }>({ totalValue: 0, totalCount: 0 });
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            try {
                ...
                // Calculate total delivery values (all time)
                const totalDeliveryValue = deliveries.reduce((sum: number, delivery: Delivery) => sum + delivery.balance, 0);
                const totalDeliveryCount = deliveries.length;
                
                setDeliveryStats({ totalValue: totalDeliveryValue, totalCount: totalDeliveryCount });
            } catch (error) {
                console.log('Delivery stats not available');
                setDeliveryStats({ totalValue: 0, totalCount: 0 });
            }
                ...
                <div className="container mx-auto px-6 py-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                        ...
                        <DeliverySummaryCard
                            ...
                            deliveryCount={deliveryStats.totalCount}
                            loading={loading}
                        />
                        <StatCard
                            title="Categories"
