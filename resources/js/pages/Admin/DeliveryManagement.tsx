...
import { Truck, Search, PlusCircle, RefreshCw, Edit, Trash2, X, Download, AlertTriangle, Clock, Calendar, CheckCircle, Eye, Package, Printer } from 'lucide-react';
import React, { useEffect, useState, useMemo } from 'react';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
import { deliveryService, Delivery, DeliveryFormData, DeliveryProduct } from '../../../services/deliveryServices';
...
const getStatusBadge = (status: Delivery['status']) => {
  ...
  return (
    <span className={`${styles[status]} text-xs font-bold px-2 py-1 rounded uppercase`}>
      {status}
    </span>
  );
};
...
const DeliveryModal = ({
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center p-4 ${!isOpen && 'hidden'}`}>
      ...
      <div className="relative w-full max-w-2xl bg-white rounded-xl shadow-2xl overflow-hidden z-10">
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSave({ ...formData, products }); }} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Reference No.</label>
              <input type="text" value={formData.reference_no} onChange={e => setFormData({...formData, reference_no: e.target.value})} className="w-full border p-2 rounded focus:ring-1 focus:ring-red-500 outline-none" required />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Customer</label>
              <input type="text" value={formData.customer} onChange={e => setFormData({...formData, customer: e.target.value})} className="w-full border p-2 rounded focus:ring-1 focus:ring-red-500 outline-none" required />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
             <div>
              ...
              <input type="text" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full border p-2 rounded focus:ring-1 focus:ring-red-500 outline-none" />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Delivery Date</label>
              <input type="date" value={formData.delivery_date} onChange={e => setFormData({...formData, delivery_date: e.target.value})} className="w-full border p-2 rounded focus:ring-1 focus:ring-red-500 outline-none" required />
            </div>
          </div>
          ...
          <div className="border rounded-lg p-4 bg-gray-50">
            <div className="flex justify-between items-center mb-4">
                ...
                {!delivery && (
                  <button type="button" onClick={addProduct} className="text-[10px] bg-gray-800 text-white px-3 py-1.5 rounded hover:bg-black transition-colors">+ Add Item</button>
                )}
            </div>
                  ...
                  <div className="grid grid-cols-12 gap-3 items-center">
                    ...
                    <div className="col-span-6">
                      ...
                      {p.searchTerm && p.searchTerm.trim() !== '' && !p.id && (
                        <div className="fixed z-50 bg-white border border-gray-300 rounded-md shadow-lg max-h-40 overflow-y-auto" 
                             style={{ 
                               width: 'calc(100% - 48px)', 
                               left: '50%', 
                               top: '50%',
                          ...
                          {inventoryItems.filter(item => 
                            ...
                            inventoryItems.filter(item => 
                              ...
                              >
                                ...
                                <div className="text-xs text-gray-500">SKU: {item.sku} • Stock: {item.stock_quantity} • Price: ₱{item.price}</div>
                              </div>
                            ))
                          ) : (
                    ...
                    <div className="col-span-2">
                      ...
                      <input 
                        ...
                        className="w-full text-sm border border-gray-300 p-2 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" 
                        required 
                        min="1"
                    ...
                    <div className="col-span-2">
                      ...
                      <input 
                        ...
                        className="w-full text-sm border border-gray-300 p-2 rounded-md bg-gray-100" 
                        readOnly 
                        required 
                        min="0"
                        step="0.01"
          ...
          <div className="grid grid-cols-3 gap-4 pt-2">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Delivery Amount</label>
              <div className="relative">
                <span className="absolute left-2 top-2 text-gray-400 text-sm">₱</span>
                <input 
                  type="number" 
                  value={formData.balance} 
                  className="w-full border p-2 pl-6 rounded bg-gray-100 font-bold" 
                  readOnly 
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Payment Received</label>
              <div className="relative">
                <span className="absolute left-2 top-2 text-gray-400 text-sm">₱</span>
                <input 
                  type="number" 
                  ...
                  className="w-full border p-2 pl-6 rounded focus:ring-1 focus:ring-red-500 outline-none" 
                />
              </div>
            </div>
            ...
            <div>
              <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Terms Payment</label>
              <div className="relative">
                <span className="absolute left-2 top-2 text-gray-400 text-sm">₱</span>
                <input 
                  type="number" 
            ...
            <div>
              ...
              {formData.status === 'paid' ? (
                ...
                <select 
                  ...
                  className="w-full border border-gray-300 p-2 rounded focus:ring-1 focus:ring-red-500 outline-none"
                >
              ...
              <div>
                ...
                <div className="relative">
                  <input 
                    ...
                    className="w-full border p-2 rounded focus:ring-1 focus:ring-red-500 outline-none bg-gray-100" 
                    readOnly 
                  />
                </div>
              </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            <button type="submit" className="px-6 py-2 bg-gray-900 text-white rounded-lg text-sm font-bold hover:bg-black transition">Save Record</button>
          </div>
        </form>
      </div>
    </div>
...
export default function DeliveryManagementPage() {
  ...
  const handleExportPDF = () => {
    ...
    if (searchTerm) {
      doc.text(`Search Term: "${searchTerm}"`, 15, 62);
    }
    ...
    // Main Data Table
    const tableColumn = ["Ref No.", "Customer", "Phone", "Products Delivered", "Date", "Expiry", "Amount", "Balance", "Status"];
    const tableRows = filtered.map(d => [
      d.reference_no,
      d.customer,
      d.phone || '-',
      d.products?.map(p => `${p.name} (x${p.quantity})`).join(', ') || 'No Items',
      d.delivery_date,
      d.expiration_date || '-',
      `P${d.balance.toFixed(2)}`,
      `P${(d.balance - (d.payment || 0)).toFixed(2)}`,
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-6">
            <div>
              ...
              <p className="text-gray-500 font-medium">H2-MED Enterprises | Logistics & Receivables</p>
            </div>
            <div className="flex gap-2">
              <button onClick={handleExportPDF} className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-bold text-gray-700 hover:bg-gray-50 shadow-sm transition">
                <Download size={16}/> Export PDF
              </button>
              <button onClick={() => { setSelectedDelivery(null); setShowModal(true); }} className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#ff1a1a] to-[#ff3333] text-white rounded-lg text-sm font-bold hover:shadow-md transition">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6">
            ...
            <div className="group relative bg-white/90 backdrop-blur-xl border border-yellow-200/60 rounded-xl p-4 shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 hover:border-yellow-300/80 overflow-hidden">
              {/* Decorative background gradient */}
              <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-yellow-200/20 to-amber-200/20 rounded-full blur-xl"></div>
              
              {/* Icon */}
              <div className="absolute top-3 right-3 relative z-10">
                <div className="bg-gradient-to-br from-yellow-400 to-yellow-500 p-2 rounded-xl shadow-md group-hover:shadow-yellow-300/50 transition-all duration-300 group-hover:scale-105">
              ...
              <div className="space-y-3 relative z-10">
                <div>
                  <p className="text-yellow-700/80 text-xs font-bold uppercase tracking-wider mb-2">Pending Deliveries</p>
                  <div className="flex items-end gap-2">
                    <div className="flex items-center gap-1.5">
                      <div className="w-1.5 h-1.5 bg-yellow-400 rounded-full animate-pulse shadow-md shadow-yellow-400/50"></div>
                      <p className="text-2xl font-black text-gray-900">{filtered.filter(d => d.status === 'pending').length}</p>
                    </div>
                    <span className="text-yellow-600/80 text-xs font-semibold mb-0.5">items</span>
                  </div>
                </div>
                
                <div className="bg-gradient-to-r from-yellow-50/80 to-amber-50/80 backdrop-blur-sm rounded-lg p-3 border border-yellow-200/50">
                  <div className="flex items-center justify-between">
                    <p className="text-yellow-700/90 text-xs font-bold uppercase tracking-wider">Total Value</p>
                    <p className="text-sm font-black text-gray-900">₱{filtered.filter(d => d.status === 'pending').reduce((sum, d) => sum + d.balance, 0).toLocaleString()}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="group relative bg-white/90 backdrop-blur-xl border border-blue-200/60 rounded-xl p-4 shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 hover:border-blue-300/80 overflow-hidden">
              {/* Decorative background gradient */}
              <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-blue-200/20 to-cyan-200/20 rounded-full blur-xl"></div>
              
              {/* Icon */}
              <div className="absolute top-3 right-3 relative z-10">
                <div className="bg-gradient-to-br from-blue-400 to-blue-500 p-2 rounded-xl shadow-md group-hover:shadow-blue-300/50 transition-all duration-300 group-hover:scale-105">
              ...
              <div className="space-y-3 relative z-10">
                <div>
                  <p className="text-blue-700/80 text-xs font-bold uppercase tracking-wider mb-2">Terms Deliveries</p>
                  <div className="flex items-end gap-2">
                    <div className="flex items-center gap-1.5">
                      <div className="w-1.5 h-1.5 bg-blue-400 rounded-full animate-pulse shadow-md shadow-blue-400/50"></div>
                      <p className="text-2xl font-black text-gray-900">{filtered.filter(d => d.status === 'terms').length}</p>
                    </div>
                    <span className="text-blue-600/80 text-xs font-semibold mb-0.5">items</span>
                  </div>
                </div>
                
                <div className="bg-gradient-to-r from-blue-50/80 to-cyan-50/80 backdrop-blur-sm rounded-lg p-3 border border-blue-200/50">
                  <div className="flex items-center justify-between">
                    <p className="text-blue-700/90 text-xs font-bold uppercase tracking-wider">Remaining Balance</p>
                    <p className="text-sm font-black text-gray-900">₱{filtered.filter(d => d.status === 'terms').reduce((sum, d) => sum + (d.balance - (d.payment || 0)), 0).toLocaleString()}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="group relative bg-white/90 backdrop-blur-xl border border-green-200/60 rounded-xl p-4 shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 hover:border-green-300/80 overflow-hidden">
              {/* Decorative background gradient */}
              ...
              {/* Icon */}
              <div className="absolute top-3 right-3 relative z-10">
                <div className="bg-gradient-to-br from-green-500 to-green-600 p-2 rounded-xl shadow-md group-hover:shadow-green-300/50 transition-all duration-300 group-hover:scale-105">
              ...
              <div className="space-y-3 relative z-10">
                <div>
                  <p className="text-green-700/90 text-xs font-bold uppercase tracking-wider mb-2">Paid Deliveries</p>
                  <div className="flex items-end gap-2">
                    <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-green-600/80 text-xs font-semibold mb-0.5">items</span>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-green-50/80 to-emerald-50/80 backdrop-blur-sm rounded-lg p-3 border border-green-200/50">
                  <div className="flex items-center justify-between">
                    <p className="text-green-700/90 text-xs font-bold uppercase tracking-wider">Total Value</p>
                    <p className="text-sm font-black text-gray-900">₱{filtered.filter(d => d.status === 'paid').reduce((sum, d) => sum + d.balance, 0).toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-12 gap-4">
              ...
              <div className="col-span-4">
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg outline-none focus:ring-2 focus:ring-[#ff1a1a] focus:border-blue-500" 
                    value={searchTerm} 
              ...
              <div className="col-span-3">
                ...
                <select 
                  ...
                  className="w-full border border-gray-300 p-2 rounded-lg focus:ring-2 focus:ring-[#ff1a1a] focus:border-blue-500"
                >
              ...
              <div className="col-span-2 flex gap-2">
                <button 
                  ...
                  className="flex-1 p-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition flex items-center justify-center"
                  title="Refresh data"
                >
              ...
              <div className="mt-4 pt-4 border-t border-gray-200">
                <div className="flex items-center justify-between">
                  <div className="text-sm text-gray-600">
                    Showing <span className="font-semibold text-blue-600">{filtered.length}</span> of {deliveries.length} deliveries
                    {statusFilter !== 'all' && <span className="ml-2">• Status: <span className="font-semibold capitalize">{statusFilter}</span></span>}
                    {deliveryDateFilter && <span className="ml-2">• Date: <span className="font-semibold">{deliveryDateFilter}</span></span>}
                    {searchTerm && <span className="ml-2">• Search: <span className="font-semibold">"{searchTerm}"</span></span>}
                  </div>
                </div>
              </div>
            )}
                    ...
                    <tr key={d.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-4 py-2">
                        {d.status === 'terms' ? (
                          <button 
                            ...
                            className="inline-flex items-center gap-2 px-3 py-1 bg-gradient-to-r from-blue-400 to-blue-500 text-white text-xs font-bold rounded uppercase hover:from-green-400 hover:to-green-500 transition-all duration-200 cursor-pointer shadow-sm hover:shadow-md"
                      ...
                      <td className="px-4 py-2 text-right">
                        <div className="flex justify-end gap-2">
                          <button onClick={() => printDeliveryReceiptDirectly(d)} className="p-1.5 text-green-600 hover:bg-green-50 rounded transition-colors" title="Print Receipt">
                            <Printer size={16}/>
                          </button>
                          <button onClick={() => { setSelectedDelivery(d); setShowModal(true); }} className="p-1.5 text-[#ff1a1a] hover:bg-red-50 rounded" title="Edit Delivery">
                            <Edit size={16}/>
                          </button>
                          <button onClick={() => handleDeleteClick(d)} className="p-1.5 text-red-600 hover:bg-red-50 rounded transition-colors" title="Delete Delivery">
                            <Trash2 size={16}/>
                          </button>
                        </div>
                      </td>
                ...
                <div className="flex gap-1">
                  {[...Array(totalPages).keys()].map((page) => {
                    ...
                    if (
                      ...
                      return (
                        <span key={pageNum} className="w-8 h-8 flex items-center justify-center text-gray-500">
                          ...
                        </span>
                      );
                    }
          ...
          {/* Success Message */}
          {successMessage && (
            <div className="fixed top-4 right-4 z-50 bg-red-50 border border-red-200 rounded-lg p-3 shadow-lg">
          ...
          <DeliveryModal isOpen={showModal} onClose={() => setShowModal(false)} delivery={selectedDelivery} onSave={handleSaveDelivery} errors={errors} setErrors={setErrors} />
            ...
            <div className="fixed inset-0 z-50 overflow-y-auto">
              <div className="flex min-h-screen items-center justify-center p-4">
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity" 
                     onClick={handleDeleteCancel}></div>
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex gap-3">
                      ...
                      <button
                        ...
                        className="flex-1 px-4 py-2.5 bg-gradient-to-r from-red-500 to-red-600 text-white rounded-lg font-medium hover:from-red-600 hover:to-red-700 transition-colors flex items-center justify-center gap-2"
                      >
                        <Trash2 size={16} />
                        Delete Permanently
                      </button>
