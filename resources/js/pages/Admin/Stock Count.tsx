...
import { Activity, RefreshCw, Download, Filter, X, TrendingUp, TrendingDown, Minus, Package, User, Calendar, FileText } from 'lucide-react';
...
const getMovementTypeBadge = (type: string) => {
    const styles: Record<string, { bg: string; text: string; icon: LucideIcon }> = {
        'in': { bg: "bg-gradient-to-r from-green-500 to-green-600", text: "text-white", icon: TrendingUp },
        'out': { bg: "bg-gradient-to-r from-red-500 to-red-600", text: "text-white", icon: TrendingDown },
        'adjustment': { bg: "bg-gradient-to-r from-blue-500 to-blue-600", text: "text-white", icon: Minus }
    };
    ...
    return (
        <span className={`${style.bg} ${style.text} text-xs font-semibold px-2 py-1 rounded-full capitalize inline-flex items-center shadow-md`}>
                ...
                <div className="relative w-full max-w-2xl transform overflow-hidden rounded-xl bg-white shadow-xl transition-all">
                    <div className="bg-gradient-to-r from-[#ff1a1a] to-[#ff3333] px-4 py-3">
                        <div className="flex items-center justify-between">
                            <h2 className="text-lg font-bold text-white">Movement Details</h2>
                            <button 
                                onClick={onClose} 
                    ...
                    <div className="p-4 space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-3">
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">Movement Type</label>
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">Stock Changes</label>
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <div className="bg-gray-50 p-3 rounded-lg">
                                        ...
                                        <p className="text-sm text-gray-600">
                                            Batch: {movement.inventory_item?.batch_number || 'N/A'}
                                        </p>
                                    </div>
                                </div>
                        ...
                        <div>
                            ...
                            <p className="text-gray-900 bg-gray-50 p-3 rounded-lg">{movement.reason}</p>
                        </div>

                        {movement.notes && (
                            <div>
                                <label className="block text-sm font-semibold text-gray-700 mb-1.5">Notes</label>
                                <p className="text-gray-900 bg-gray-50 p-3 rounded-lg">{movement.notes}</p>
                            </div>
                        )}
...
export default function StockMovementPage() {
    ...
    }, [pagination.per_page, typeFilter, dateFromFilter, dateToFilter]);
                        ...
                        <table>
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 10%;">Stock Change</th>
                            ...
                            <tbody>
                                ${movements.map((movement) => `
                                    <tr>
                                        ...
                                        <td><strong>${movement.inventory_item?.batch_number || 'N/A'}</strong></td>
                                        <td>
                ...
                <div className="container mx-auto px-4 py-6">
                    <div className="mb-6 flex flex-col md:flex-row md:items-center md:justify-between">
                        <div className="flex items-center mb-4 md:mb-0">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-gray-600">Track all inventory movements and changes</p>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-6">
                        <div className="bg-white rounded-xl shadow-md p-4 border border-gray-100">
                            <div className="text-xs font-medium text-gray-500 mb-1.5">Total Movements</div>
                            <div className="text-2xl font-bold text-gray-900">{stats.total_movements}</div>
                        </div>
                        <div className="bg-white rounded-xl shadow-md p-4 border border-gray-100">
                            <div className="text-xs font-medium text-gray-500 mb-1.5">Stock In Today</div>
                            <div className="text-2xl font-bold text-green-600">{stats.stock_in_today}</div>
                        </div>
                        <div className="bg-white rounded-xl shadow-md p-4 border border-gray-100">
                            <div className="text-xs font-medium text-gray-500 mb-1.5">Stock Out Today</div>
                            <div className="text-2xl font-bold text-red-600">{stats.stock_out_today}</div>
                        </div>
                        <div className="bg-white rounded-xl shadow-md p-4 border border-gray-100">
                            <div className="text-xs font-medium text-gray-500 mb-1.5">Adjustments Today</div>
                            <div className="text-2xl font-bold text-blue-600">{stats.adjustments_today}</div>
                        </div>
                        <div className="bg-white rounded-xl shadow-md p-4 border border-gray-100">
                            <div className="text-xs font-medium text-gray-500 mb-1.5">This Week</div>
                            <div className="text-2xl font-bold text-[#ff1a1a]">{stats.movements_this_week}</div>
                        </div>
                        <div className="bg-white rounded-xl shadow-md p-4 border border-gray-100">
                            <div className="text-xs font-medium text-gray-500 mb-1.5">This Month</div>
                            <div className="text-2xl font-bold text-[#ff1a1a]">{stats.movements_this_month}</div>
                        </div>
                    </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
                                    <tr>
                                        <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Product Name</th>
                                        <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Batch Number</th>
                                        <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Type</th>
                                        <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Stock Change</th>
                                        <th className="px-4 py-3 text-left text-xs font-bold text-gray-700 uppercase tracking-wider">Available</th>
                                    ...
                                    {loading ? (
                                        ...
                                        movements.map((movement) => (
                                            <tr key={movement.id} className="hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="px-4 py-3 whitespace-nowrap">
                                                    <div className="text-sm font-medium text-gray-900">
                                                        {movement.inventory_item?.batch_number || 'N/A'}
                                                    </div>
                                                </td>
                                                <td className="px-4 py-3 whitespace-nowrap">
                                                ...
                                                <td className="px-4 py-3 whitespace-nowrap text-right">
                                                    <button
                                                        ...
                                                        className="inline-flex items-center px-2 py-1.5 text-sm font-medium text-[#ff1a1a] hover:bg-red-50 rounded-lg transition-colors"
                                                        title="View Details"
                                                    >
